...
export default function SellerDashboardProducts() {
  ...
  const productStats = {
    ...
    inStock: products.reduce((acc, product) => acc + (product.stock || 0), 0),
    outOfStock: products.filter(product => (product.stock || 0) === 0).length,
    totalSales: storeData.totalOrders || 0
  }
  ...
  // FIXED: Single useEffect for filtering that handles all filter logic
  useEffect(() => {
    // Safety check 
    ...
    // If selectedAvailability === "all", no additional filtering needed
  ...
  const confirmDelete = async () => {
    try {
      // Delete from backend
      await axios.post('/api/admin/delete-product', { productId: productToDelete.id })
  ...
  if (error === true) {
    return (
      <Layout>
        <div className="w-full h-screen flex justify-center pt-[20rem] text-[14px] text-gray-600">
              ...
              {currentProducts.length > 0 ? (
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-8 text-center">
                  ...
                  <button 
                    ...
                    className="text-orange hover:text-gray-700 font-medium"
                  >
                    Clear all filters
                  </button>
                </div>
